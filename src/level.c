...
LEVELRun(LEVELDESC* Lvl)
{
  ...
  while (Quit == 0) // While level neither abandoned nor completed
  {
    ...
    n = 0;
    y = MAXY; // Start with furthest object
    do
    {
      TREEDrawAll( // Draw up to that object - or the nearest if it exists
        &Trees,
        Objects[n].y,
        y);             // there will be at least one -MAXY unused object post sort
    ...
    if (n & KEYEXIT)
      Quit = Escaped = 1; // Exit key/button pressed
    ...
    if (MissileSpr >= 0) // Missile still in progress ?
    {
      if (OBJCheckHitEnemy(Objects, // If hit one of the bad guys, cancel missile
                           Objects[MissileSpr].x,
                           Objects[MissileSpr].y))
      {
  ...
  if (Glo.Score > Glo.HighScore)
    Glo.HighScore = Glo.Score;
  return Escaped;
}
